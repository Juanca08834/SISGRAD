...
public class EncriptadorContrasenia {
    ...
    public boolean verificar(char[] contrasenia, String hashGuardado) {
        try {
            ...
            if (hashCalc.length != hashEsperado.length) {
                return false;
            }
            int diff = 0;
            for (int i = 0; i < hashCalc.length; i++) {
                diff |= hashCalc[i] ^ hashEsperado[i];
            }
            return diff == 0;
        } catch (Exception e) {
            return false;
    ...
    public String hashear(String contraseniaPlana) {
        char[] tmp = contraseniaPlana != null ? contraseniaPlana.toCharArray() : null;
        try {
            return generarHash(tmp);
        } finally {
            if (tmp != null) {
                java.util.Arrays.fill(tmp, '\0'); // higiene de memoria
            }
        }
    }

    public boolean validar(String contraseniaPlana, String hashGuardado) {
        char[] tmp = contraseniaPlana != null ? contraseniaPlana.toCharArray() : null;
        try {
            return verificar(tmp, hashGuardado);
        } finally {
            if (tmp != null) {
